	...
	if(typeof aimgcount == 'object' && $('imagelistthumb_' + pid)) {
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				if(fid) {
					imagelist += '<div id="pattimg_' + aimgcount[pid][i] + '_menu" class="tip tip_4" style="display: none;"><div class="tip_horn"></div><div class="tip_c"><a href="http://www.17sucai.com//preview/869565/2017-11-13/js/style/js/forum.php?mod=ajax&action=setthreadcover&aid=' + aimgcount[pid][i] + '&fid=' + fid + '" class="xi2" onclick="showWindow(\'setcover' + aimgcount[pid][i] + '\', this.href)">设为封面</a></div></div>';
				}
...
function parsetag(pid) {
	...
	var str = $('postmessage_'+pid).innerHTML.replace(/(^|>)([^<]+)(?=<|$)/ig, function($1, $2, $3, $4) {
		...
		$('postmessage_'+pid).innerHTML = str.replace(/<h_ (\d+)>/ig, function($1, $2) {
			return '<span href=\"http://www.17sucai.com//preview/869565/2017-11-13/js/style/js/forum.php?mod=tag&name=' + tagencarray[$2] + '\" onclick=\"tagshow(event)\" class=\"t_tag\">' + tagfindarray[$2] + '</span>';
	    	});
	}
}
...
function fastpostappendreply() {
	if($('fastpostrefresh') != null) {
		...
		if($('fastpostrefresh').checked) {
			location.href = 'http://www.17sucai.com//preview/869565/2017-11-13/js/style/js/forum.php?mod=redirect&tid='+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
			return;
		}
	}
...
function submitpostpw(pid, tid) {
	...
	safescript('md5_js', function () {
		...
		if(!tid) {
			...
			location.href = 'http://www.17sucai.com//preview/869565/2017-11-13/js/style/js/forum.php?mod=viewthread&tid='+tid;
		}
	}, 100, 50);
}
...
function vmessage() {
	...
	$('vreplysubmit').onmouseover = function() {
		if($('vmessage').value != vf_tips) {
			ajaxget('http://www.17sucai.com//preview/869565/2017-11-13/js/style/js/forum.php?mod=ajax&action=checkpostrule&ac=reply', 'vfastpostseccheck');
